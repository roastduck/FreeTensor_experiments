...
&         image_size*image_size)+pn) = soft_color(k)/softmax_sum
    ...
    DO bn=batch_size,1,-1
      DO pn=image_size*image_size,1,-1
        softmax_sumb = 0.0_8
        DO k=3,1,-1
...
&           softmax_sum
          softmax_sumb = softmax_sumb - soft_color(k)*soft_colors_tb((bn&
&           -1)*(4*image_size*image_size)+(k-1)*(image_size*image_size)+&
&           pn)/softmax_sum**2
          soft_colors_tb((bn-1)*(4*image_size*image_size)+(k-1)*(&
&         image_size*image_size)+pn) = 0.0_8
        END DO
        CALL POPREAL8(soft_colors_t((bn-1)*(4*image_size*image_size)+(4-&
&               1)*(image_size*image_size)+pn))
...
&               soft_color_alphab(fn)
          ...
          END IF
          CALL POPREAL8(dis)
          CALL EUCLIDEAN_P2F_DISTANCE_B(faces_t, faces_tb, pixel, fn, bn&
&                                 , disb)
          CALL POPCONTROL1B(branch)
          IF (branch .EQ. 0) THEN
            CALL POPREAL8(sign)
...
&           soft_colorb(k)
        ...
        END DO
        CALL POPREAL8(softmax_sum)
        softmax_maxb = softmax_maxb - EXP((2*eps-softmax_max)/gamma_val)&
&         *softmax_sumb/gamma_val
        DO fn=num_faces,1,-1
          CALL POPCONTROL3B(branch)
          IF (branch .LT. 2) THEN
            IF (branch .EQ. 0) THEN
