...
contains
    ...
    subroutine face_inv(inv_t, faces_t, fn, bn)
        implicit none
        real*8, intent(in) :: faces_t(3 * 3 * num_faces * batch_size)
    ...
    logical function check_border(p, faces_t, threshold_t, fn, bn)
        real(8), intent(in) :: p(3), faces_t(3 * 3 * num_faces * batch_size)
        real(8), intent(in) :: threshold_t
        integer, intent(in) :: fn, bn
        integer :: d
        real(8) :: t
        d = (bn - 1) * (num_faces * 3 * 3) + (fn - 1) * (3 * 3) + 1
    ...
    real(8) function euclidean_p2f_distance(faces_t, p, fn, bn)
        real(8), intent(in) :: faces_t(3 * 3 * num_faces * batch_size), p(3)
        integer, intent(in) :: fn, bn
        real(8) :: dis(3)
    ...
    real(8) function forward_sample_texture(textures_t, w, r, k, ty, fn, bn)
        real(8), intent(in) :: textures_t(3 * texture_size * num_faces * batch_size), w(3)
    ...
    subroutine tapenade_render_main(soft_colors_t, faces_t, textures_t) BIND(c)
        real(8), intent(inout) :: soft_colors_t(image_size * image_size * 4 * batch_size)
        real(8), intent(in) :: faces_t(3 * 3 * num_faces * batch_size)
        real(8), intent(in) :: textures_t(3 * texture_size * num_faces * batch_size)
        ...
        real(8), dimension(num_faces) :: soft_color_alpha
        real(8) :: sign, softmax_max, softmax_sum, zp, zp_norm, coef, color_k, soft_fragment, dis
        do bn = 1, batch_size
!$OMP PARALLEL DO SHARED(image_size, batch_size, num_faces, texture_size, soft_colors_t, faces_t, textures_t &
!$OMP&, bn, texture_res, near, far, eps, sigma_val, gamma_val, dist_eps, threshold, double_side, texture_type) &
!$OMP&, PRIVATE(pn, yi, xi, fn, k, d, pixel, inv, w, w_clip, soft_color, soft_color_alpha, sign, softmax_max &
!$OMP&, softmax_sum, zp, zp_norm, coef, color_k, soft_fragment, dis)
