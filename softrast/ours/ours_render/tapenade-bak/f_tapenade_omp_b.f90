...
!   with respect to varying inputs: faces_t inv_t
  SUBROUTINE FACE_INV_B(inv_t, inv_tb, faces_t, faces_tb, fn, bn)
    ...
    REAL*8 :: faces_tb(3*3*num_faces*batch_size)
...
!   with respect to varying inputs: w w_clip
  SUBROUTINE BARYCENTRIC_CLIP_B(w_clip, w_clipb, w, wb)
    ...
    REAL*8 :: x1
    REAL*8 :: x1b
    REAL*8, DIMENSION(3) :: ad_save
    REAL*8, DIMENSION(3) :: ad_save0
...
&   euclidean_p2f_distanceb)
    ...
    REAL*8 :: max1
    REAL*8 :: max1b
    REAL*8, DIMENSION(3, 3) :: ad_save
    REAL*8, DIMENSION(3) :: pb
    REAL*8, DIMENSION(3, 3) :: ad_save0
    REAL*8, DIMENSION(3) :: ad_save1
    REAL*8, DIMENSION(3, 3) :: ad_save2
    REAL*8, DIMENSION(3) :: ad_save3
    REAL*8, DIMENSION(3) :: ad_save4
    INTEGER, DIMENSION(3) :: ad_branch
    INTEGER, DIMENSION(3) :: ad_branch0
    INTEGER :: ad_branch1
...
&   ), w(3)
    REAL*8 :: textures_tb(3*texture_size*num_faces*batch_size)
...
&   faces_t, faces_tb, textures_t, textures_tb) BIND(c)
    IMPLICIT NONE
    REAL*8, INTENT(INOUT) :: soft_colors_t(image_size*image_size*4*&
&   batch_size)
    REAL*8, INTENT(INOUT) :: soft_colors_tb(image_size*image_size*4*&
&   batch_size)
    REAL*8, INTENT(IN) :: faces_t(3*3*num_faces*batch_size)
    REAL*8 :: faces_tb(3*3*num_faces*batch_size)
    REAL*8, INTENT(IN) :: textures_t(3*texture_size*num_faces*batch_size&
&   )
    ...
    REAL*8, DIMENSION(num_faces) :: soft_color_alphab
...
&                     , texture_res, k, texture_type, fn, bn)
          ...
          END IF
        END DO
        CALL PUSHREAL8(soft_colors_t((bn-1)*(4*image_size*image_size)+(4&
...
&       *image_size)+pn) = 1.0 - soft_color_alpha(num_faces)
        DO k=1,3
          CALL PUSHREAL8(soft_colors_t((bn-1)*(4*image_size*image_size)+&
...
&         image_size*image_size)+pn) = soft_color(k)/softmax_sum
      ...
      CALL PUSHREAL8(sign)
      CALL PUSHREAL8(dis)
      CALL PUSHREAL8ARRAY(pixel, 3)
      CALL PUSHREAL8(color_k)
      CALL PUSHREAL8(soft_fragment)
      CALL PUSHREAL8ARRAY(w_clip, 3)
      CALL PUSHREAL8ARRAY(soft_color, 3)
      CALL PUSHREAL8ARRAY(w, 3)
      CALL PUSHREAL8(zp_norm)
      CALL PUSHREAL8ARRAY(soft_color_alpha, num_faces)
      CALL PUSHREAL8(coef)
      CALL PUSHREAL8(softmax_max)
!$OMP END PARALLEL
    END DO
...
!$OMP&, chunk_start)
      ...
      DO ichunk=1,numchunks
        ...
        DO pn=chunk_end,chunk_start,-1
          softmax_sumb = 0.0_8
          DO k=3,1,-1
            CALL POPREAL8(soft_colors_t((bn-1)*(4*image_size*image_size)&
&                   +(k-1)*(image_size*image_size)+pn))
...
&           image_size*image_size)+pn) = 0.0_8
          END DO
          CALL POPREAL8(soft_colors_t((bn-1)*(4*image_size*image_size)+(&
