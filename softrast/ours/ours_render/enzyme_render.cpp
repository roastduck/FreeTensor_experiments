...
extern void __enzyme_autodiff_render(void *, int, scalar_t *, const scalar_t *,
                                     int, const scalar_t *, scalar_t *, int,
...
scalar_t *soft_colors, *d_soft_colors;
...
void read_faces(scalar_t *a, std::string file) {
...
void read_textures(scalar_t *a, std::string file) {
    freopen(file.c_str(), "r", stdin);
    size_t batchSize, numFaces, x, y;
    std::cin >> batchSize >> numFaces >> x >> y;
    assert(batchSize == batch_size);
...
void read_grad_soft_colors(scalar_t *a, std::string file) {
    freopen(file.c_str(), "r", stdin);
    size_t batchSize, numFaces, x, y;
    std::cin >> batchSize >> numFaces >> x >> y;
    assert(batchSize == batch_size);
...
int main(int argc, char **argv) {
    ...
    read_faces(faces, std::string("./data/face_vertices") + argv[1]);
    read_textures(textures, std::string("./data/face_textures") + argv[1]);
    /*read_grad_soft_colors(
        d_soft_colors,
        "./data/iter2023_b256_i64_n1280_t1/grad_soft_colors.txt");*/

    image_size = atoi(argv[2]);
    size_t size = batch_size * 4 * image_size * image_size;
    soft_colors = new scalar_t[size];
    d_soft_colors = new scalar_t[size];
    for (size_t i = 0; i < size; i++) {
        d_soft_colors[i] = 1.;
    }
    ...
    print(soft_colors,
          std::string("./result/enzyme_soft_colors") + argv[1] + "_" + argv[2] +
              ".txt",
          {batch_size, 4, image_size, image_size});
    print(d_faces,
          std::string("./result/enzyme_grad_faces") + argv[1] + "_" + argv[2] +
              ".txt",
          {batch_size, num_faces, 3, 3});
    print(d_textures,
          std::string("./result/enzyme_grad_textures") + argv[1] + "_" +
              argv[2] + ".txt",
