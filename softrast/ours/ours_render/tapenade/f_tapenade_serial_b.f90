...
&   euclidean_p2f_distanceb)
    ...
    DO k=1,3
      ...
      CALL PUSHREAL4ARRAY(t1, 3)
      CALL SUB_XY(t1, p, tmp0)
      CALL PUSHREAL4ARRAY(t2, 3)
      CALL SUB_XY(t2, tmp1, tmp0)
      CALL PUSHREAL4(area)
      area = CROSS_XY(t1, t2)
      d1 = DOT_XY(t1, t2)
      IF (d1 .GE. 0.0) THEN
        CALL PUSHREAL4ARRAY(t3, 3)
...
&         image_size*image_size)+pn) = 0.0_4
        END DO
        CALL POPREAL4(soft_colors_t((bn-1)*(4*image_size*image_size)+(4-&
&               1)*(image_size*image_size)+pn))
