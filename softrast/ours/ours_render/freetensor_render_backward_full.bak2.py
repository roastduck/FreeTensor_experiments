    ...
    def our_render_main(faces, textures, soft_colors):
        ...
        for bn in range(batch_size):
            for pn in range(image_size * image_size):
                ...
                #! label: softmax_sum
                softmax_sum = ft.empty((), "float32")
                softmax_sum[()] = ft.exp((background_weight - softmax_max) / gamma_val)
...
def our_render(faces, textures):
    ...
    d_soft_colors = ft.array(np.ones(soft_colors.shape, dtype="float32"))

    d_faces = ft.array(np.zeros(faces.shape, dtype="float32"))
    d_textures = ft.array(np.zeros(textures.shape, dtype="float32"))
